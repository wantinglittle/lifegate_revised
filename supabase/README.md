...
- `supabase/migrations/20260724210000_grant_service_role_group_owner_update.sql`
...
`supabase/functions/submit-group/index.ts` is the Supabase replacement for the former Firebase `submitGroup` Cloud Function. It accepts browser `POST` submissions, handles harmless `OPTIONS` preflight requests, verifies reCAPTCHA server-side, validates the same form fields, inserts one pending row into `public.groups`, and then attempts the EmailJS notification.

The production `add-group.js` form points to the deployed Supabase Edge Function. The legacy Firebase project remains available for rollback/reference until parity testing is complete.
...
For production Netlify, set these values in Supabase Dashboard -> Authentication -> URL Configuration:
...
Keep the old GitHub Pages callback URL temporarily only if previously sent magic links or invitations may still use it:
