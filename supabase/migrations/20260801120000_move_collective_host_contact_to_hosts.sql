...
alter table public.collective_hosts
  add column if not exists phone text null;

comment on column public.collective_hosts.phone is
  'Relationship-specific Collective host phone number. Primary host phones are backfilled from collectives.primary_host_phone.';

update public.collective_hosts as ch
set phone = nullif(btrim(c.primary_host_phone), '')
from public.collectives as c
where ch.collective_id = c.id
  and ch.is_primary = true
  and nullif(btrim(ch.phone), '') is null
  and nullif(btrim(c.primary_host_phone), '') is not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'collective_hosts_phone_not_blank'
      and conrelid = 'public.collective_hosts'::regclass
  ) then
    alter table public.collective_hosts
      add constraint collective_hosts_phone_not_blank
